...
static int validate_file(std::string &path, const Server &server, std::string &file_location, std::ifstream &file)
{
...
bool	httpRequest::getResponse(const Server &server, std::string &content_type, bool is_cgi_script)
{
  ...
  int status_code = validate_file(target_path, server, file_location, file);
  if (status_code != 200)
  {
